...
import TemplateCoverThumbnail, { Lines } from "./TemplateCoverThumbnail";
...
/* ─── Thumbnails ─────────────────────────────────────────────── */
/* Las portadas vienen del componente compartido TemplateCoverThumbnail */
...
export default function TemplatePreviewModal({ template, onClose, onSelect, selected }: Props) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 modal-overlay${hiding ? " hiding" : ""}`}
      style={{ background: "rgba(0,0,0,.75)" }}
      onClick={close}
    >
      <div
        className={`bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl modal-panel${hiding ? " hiding" : ""}`}
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          ...
          <button onClick={close} className="text-gray-500 hover:text-gray-300 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <div style={PAGE_STYLE}>
                <TemplateCoverThumbnail template={template} />
              </div>
            </div>
            {/* Contenido */}
            <div>
        ...
        <div className="px-6 py-4 border-t border-gray-800 flex items-center justify-between gap-3">
          ...
          <button
            onClick={() => { onSelect(template); close(); }}
